...
const ClientRequests = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Filter: Only show requests belonging to the logged-in user
  ...
  const handleQuote = async (id: string, action: 'Approved' | 'Rejected') => {
    try {
      ...
      // Make sure your backend has a PATCH /update/:id route
      await axios.patch(`${API_BASE_URL}/update/${id}`, { status: newStatus });
  ...
  // Quotes awaiting approval (Backend Enum: 'quote sent')
            ...
            {pendingQuotes.map(q => (
              <div key={q._id} className="flex items-center justify-between p-4 rounded-xl bg-card border shadow-sm">
                <div>
                  ...
                  <p className="text-2xl font-black mt-1">${q.fees?.toLocaleString() || '0'}</p>
                </div>
